...
class IFile
{
	...
	{
		$dir         .= IWeb::$app->config['encryptKey'];
		$encodeString = ICrypt::base64encode($dir);
		return strtr($encodeString,array('+' => '-','/' => '_','=' => ''));
	}
	...
	{
		$code = strtr($code,array('-' => '+','_' => '/'));
		$mod4 = strlen($code) % 4;
		if($mod4)
		{
			$code .= substr('====', $mod4);
		}
		$dir = ICrypt::base64decode($code);
		return strstr($dir,IWeb::$app->config['encryptKey'],true);
	}
}