...
<body style="width:700px;min-height:400px;overflow-y:hidden;">
<div class="pop_win">
		...
		<tr>
			<td>
				...
				<div class="cont" style='height:285px;'>
					...
					<li>
						<label><input type="radio" autocomplete="off" name="spec" onclick="selSpect(this,<%=item['id']%>)" value='<%=JSON().stringify(item)%>' /><%=item['name']%><%if(item['note']){%>[<%=item['note']%>]<%}%></label>
					</li>
					</script>
				</div>
				<p>没有找到需要的规格？</p>
			...
			<td>
				...
				<div class="cont" style="width:360px;">
					...
					<%var valueList = JSON().parse(templateData['value']);%>
